...
class NPCHuman extends Human {
	...
	public function onUpdate($tick){
		...
		$this->setDataProperty(2, 4, TFC::center(str_ireplace(["{o}", "{tps}", "{c}", "{date}", "{hh:mm:ss}", "{top1}", "{top2}", "{top3}"], [count($this->server->getOnlinePlayers()), $this->server->getTicksPerSecond(), count($players), date("d/m/Y"), date("H:i:s"), $main->getTop(1), $main->getTop(2), $main->getTop(3)], $this->namedtag["Name"])));
